...
from .backgrounds import Background, available_people, build_background
...
SCHEMA = {
    ...
    "properties": {
        ...
        "people": {"type": "array", "items": {"type": "string"}, "description": "초상 라이브러리 키, 최대 2명"},
        "bg_query": {"type": "string", "description": "배경 사진 검색용 영어 키워드 2~4단어"},
    },
    "required": ["headline", "blog_cards", "related_cards", "caption_hook", "hashtags", "people", "bg_query"],
    "additionalProperties": False,
}
...
class BlogBriefing:
    ...
    hashtags: list[str]
    people: tuple[str, ...] = ()
    """초상 라이브러리 키 (최대 2명). 표지 배경 콜라주에 쓴다."""
    bg_query: str = ""
    """인물이 없을 때 Unsplash에서 배경을 찾을 영어 검색어."""
    input_tokens: int = 0
    output_tokens: int = 0
    dropped_cards: int = 0
...
def summarize_blog(
    ...
    # 모델이 라이브러리에 없는 인물 키를 지어내도 발행이 죽지 않도록 조용히 거른다.
    known = available_people()
    people = tuple(key for key in payload["people"] if key in known)[:2]
...
def build_caption(briefing: BlogBriefing, credits: tuple[str, ...] = ()) -> str:
    lines = [
    ...
    if credits:
        # CC BY 사진은 크레딧 표기가 라이선스 조건이다. 빼면 안 된다.
...
def render_blog(
    ...
    images = [
        render_cover(
            briefing.headline,
            briefing.post.published,
            fonts,
            kicker=f"{BLOG_NAME} 인사이트",
    ...
    (target / "caption.txt").write_text(build_caption(briefing, credits), encoding="utf-8")
    return paths
...
def main() -> int:
    ...
    for i, card in enumerate(briefing.cards, 1):
        ...
        print(f"   {card.body}\n")
    bg_errors: list[str] = []
    background = build_background(list(briefing.people), briefing.bg_query, errors=bg_errors)
    if briefing.people:
        print(f"표지 배경: 인물 콜라주 {briefing.people}")
    elif background:
        print(f"표지 배경: Unsplash '{briefing.bg_query}' ({', '.join(background.credits)})")
    else:
        print(f"표지 배경: 단색 폴백 (bg_query='{briefing.bg_query}')")
    for message in bg_errors:
        print(f"  [경고] {message}")

    print("--- 캡션 ---")
    print(build_caption(briefing, background.credits if background else ()))

    if briefing.dropped_cards:
        print(f"\n[경고] 수치 검증에 걸려 카드 {briefing.dropped_cards}장을 폐기했습니다.")

    paths = render_blog(briefing, background=background)
