...
from econ_insta.reels import HEIGHT, WIDTH, build_stock_reel
from econ_insta.renderer import BG_COVER, FontSet
...
# PD-textlogo(단순 텍스트라 저작권 없음) — 보도 맥락에서 회사를 지칭하는 상표 사용.
LOGO_URL = (
    "https://upload.wikimedia.org/wikipedia/commons/thumb/2/24/"
    "SK_Hynix.svg/1280px-SK_Hynix.svg.png"
)


def logo_background() -> Image.Image | None:
    """다크 표지 위에 SK하이닉스 로고를 올린 배경. 실패하면 단색으로 나간다."""
    try:
        response = requests.get(LOGO_URL, timeout=30, headers={"User-Agent": "econ-insta/1.0"})
        response.raise_for_status()
        logo = Image.open(BytesIO(response.content)).convert("RGBA")
    except (requests.RequestException, OSError) as exc:
        print(f"  ! 로고 내려받기 실패 — 단색 표지로 나갑니다 ({exc})")
        return None

    canvas = Image.new("RGB", (WIDTH, HEIGHT), BG_COVER)
    target = int(WIDTH * 0.62)
    logo.thumbnail((target, target), Image.LANCZOS)
    # scene_cover의 그라디언트가 상·하단을 누르므로, 밝게 남는 중상단에 둔다.
    canvas.paste(logo, ((WIDTH - logo.width) // 2, 620), logo)
    return canvas
